...
class ffnet:
    ...
    def setin(self):
        ...
        for k in range(1, len(self.inno) + 1):
            self.units[self.inno[k]] = (
                self.eni[k][1] * self.input[k - 1] + self.eni[k][2]
            )  # because self.input is a python list, and the others were inputted from a fortran-format file
...
class atom:
    ...
    def SideChainOrBackBone(
        self,
    ):  # only really applies to proteins, assuming standard atom names
        if (
            self.atomname.strip() == "CA"
            or self.atomname.strip() == "C"
            or self.atomname.strip() == "O"
            or self.atomname.strip() == "N"
        ):
            return "BACKBONE"
        else:
            return "SIDECHAIN"
    ...
    def ReadPDBLine(self, Line):
        ...
        if len(self.atomname) == 1:
            ...
            self.atomname = (
                self.atomname + " "
            )  # This line is necessary to work, though many PDBs in the PDB would have this line commented out
        ...
        self.residue = (
            " " + self.residue[-3:]
        )  # this only uses the rightmost three characters, essentially removing unique rotamer identification
...
class PDB:
        ...
        for t in range(0, len(lines)):
            ...
            if len(line) >= 7:
                if (
                    ...
                    if (
                        not key in atom_already_loaded
                        or not TempAtom.residue.strip() in self.protein_resnames
                    ):  # so either the atom hasn't been loaded, or else it's a non-receptor atom
    ...
    def SavePDB(self, filename):
        ...
        if towrite.strip() == "":
            towrite = "ATOM      1  X   XXX             0.000   0.000   0.000                       X"  # just so no PDB is empty, VMD will load them all
        f.write(towrite)
        f.close()
    ...
    def SavePDBString(self):
        ...
        for atomindex in self.AllAtoms:
            ToOutput = (
                ToOutput + self.AllAtoms[atomindex].CreatePDBLine(atomindex) + "\n"
            )
                ...
                for AtomIndex2 in self.NonProteinAtoms:
                    if AtomIndex1 != AtomIndex2:
                        ...
                        if (
                            ...
                            if (
                                dist
                                < self.BondLength(atom1.element, atom2.element) * 1.2
                            ):
    ...
    def BondLength(self, element1, element2):
        ...
        if (element1 == "S" and element2 == "H") or (
            ...
            distance = (
                2.025 / 1.5
            )  # This one not from source sited above. Not sure where it's from, but it wouldn't ever be used in the current context ("AutoGrow")
        if (element1 == "S" and element2 == "N") or (
            element1 == "N" and element2 == "S"
        ):
            distance = 1.633
        ...
        if (element1 == "P" and element2 == "O") or (
            ...
            distance = (
                1.6  # estimate based on eye balling Handbook of Chemistry and Physics
            )
        ...
        for atom_index in self.NonProteinAtoms:
            ...
            if atom.element == "N":
                if atom.NumberOfNeighbors() == 4:  # a quartinary amine, so it's easy
                    ...
                    chrg = self.charged(
                        atom.coordinates, indexes, True
                    )  # so the indicies stored is just the index of the nitrogen and any attached atoms
                    self.charges.append(chrg)
                elif (
                    atom.NumberOfNeighbors() == 3
                ):  # maybe you only have two hydrogen's added, by they're sp3 hybridized. Just count this as a quartinary amine, since I think the positive charge would be stabalized.
                    nitrogen = atom
                    ...
                    if math.fabs(average_angle - 109.0) < 5.0:
                        ...
                        indexes.extend(atom.IndeciesOfAtomsConnecting)
                        chrg = self.charged(
                            nitrogen.coordinates, indexes, True
                        )  # so indexes added are the nitrogen and any attached atoms.
                        self.charges.append(chrg)

            if (
                atom.element == "C"
            ):  # let's check for guanidino-like groups (actually H2N-C-NH2, where not CN3.)
                if (
                    atom.NumberOfNeighbors() == 3
                    ...
                    if (
                        ...
                        if len(all_connected) > 0:
                            not_isolated = all_connected[
                                0
                            ]  # get the atom that connects this charged group to the rest of the molecule, ultimately to make sure it's sp3 hybridized

                        if (
                            len(nitrogens_to_use) == 2 and not_isolated != -1
                        ):  # so there are at two nitrogens that are only connected to the carbon (and probably some hydrogens)
                            ...
                            if (
                                (
                                    ...
                                    and not_isolated_atom.NumberOfNeighbors() == 4
                                )
                                or (
                                ...
                                pt.x = (
                                    pt.x
                                    + self.AllAtoms[nitrogens_to_use[1]].coordinates.x
                                )
                                pt.y = (
                                    pt.y
                                    + self.AllAtoms[nitrogens_to_use[1]].coordinates.y
                                )
                                pt.z = (
                                    pt.z
                                    + self.AllAtoms[nitrogens_to_use[1]].coordinates.z
                                )
                                pt.x = pt.x / 2.0
                                pt.y = pt.y / 2.0
                                pt.z = pt.z / 2.0
            ...
            if (
                atom.element == "P"
            ):  # let's check for a phosphate or anything where a phosphorus is bound to two oxygens where both oxygens are bound to only one heavy atom (the phosphorus). I think this will get several phosphorus substances.
                oxygens = self.connected_atoms_of_given_element(atom_index, "O")
                if len(oxygens) >= 2:  # the phosphorus is bound to at least two oxygens
                    ...
                    if (
                        count >= 2
                    ):  # so there are at least two oxygens that are only bound to the phosphorus
                        indexes = [atom_index]
            ...
            if (
                atom.element == "S"
            ):  # let's check for a sulfonate or anything where a sulfur is bound to at least three oxygens and at least three are bound to only the sulfur (or the sulfur and a hydrogen).
                oxygens = self.connected_atoms_of_given_element(atom_index, "O")
                if len(oxygens) >= 3:  # the sulfur is bound to at least three oxygens
                    ...
                    if (
                        count >= 3
                    ):  # so there are at least three oxygens that are only bound to the sulfur
                        indexes = [atom_index]
                        indexes.extend(oxygens)
                        chrg = self.charged(atom.coordinates, indexes, False)
                        self.charges.append(chrg)
    ...
    def assign_charged_from_protein_process_residue(self, residue, last_key):
        ...
        if (
            real_resname == "LYS" or real_resname == "LYN"
            ...
            for index in residue:
                ...
                if atom.atomname.strip() == "NZ":
                    ...
                    for index2 in residue:
                        atom2 = self.AllAtoms[index2]
                        if atom2.atomname.strip() == "HZ1":
                            indexes.append(index2)
                        if atom2.atomname.strip() == "HZ2":
                            indexes.append(index2)
                        if atom2.atomname.strip() == "HZ3":
                            indexes.append(index2)
        ...
        if real_resname == "ARG":
            ...
            for index in residue:
                ...
                if atom.atomname.strip() == "NH2":
                    ...
                    indices.append(index)
                    count = count + 1.0
                if atom.atomname.strip() == "2HH2":
                    indices.append(index)
                if atom.atomname.strip() == "1HH2":
                    indices.append(index)
                if atom.atomname.strip() == "CZ":
                    indices.append(index)
                if atom.atomname.strip() == "2HH1":
                    indices.append(index)
                if atom.atomname.strip() == "1HH1":
                    indices.append(index)
        ...
        if (
            ...
            or real_resname == "HIE"
            or real_resname == "HIP"
        ):  # regardless of protonation state, assume it's charged. This based on "The Cation-Pi Interaction," which suggests protonated state would be stabalized. But let's not consider HIS when doing salt bridges.
            charge_pt = point(0.0, 0.0, 0.0)
            count = 0.0
            indices = []
            for index in residue:
                ...
                if atom.atomname.strip() == "ND1":
                    ...
                    indices.append(index)
                    count = count + 1.0
                if atom.atomname.strip() == "HE2":
                    indices.append(index)
                if atom.atomname.strip() == "HD1":
                    indices.append(index)
                if atom.atomname.strip() == "CE1":
                    indices.append(index)
                if atom.atomname.strip() == "CD2":
                    indices.append(index)
                if atom.atomname.strip() == "CG":
                    indices.append(index)
        ...
        if (
            real_resname == "GLU" or real_resname == "GLH" or real_resname == "GLX"
        ):  # regardless of protonation state, assume it's charged. This based on "The Cation-Pi Interaction," which suggests protonated state would be stabalized.
            charge_pt = point(0.0, 0.0, 0.0)
            count = 0.0
            indices = []
            for index in residue:
            ...
            if count != 0.0:
                ...
                if charge_pt.x != 0.0 or charge_pt.y != 0.0 or charge_pt.z != 0.0:
                    chrg = self.charged(
                        charge_pt, indices, False
                    )  # False because it's a negative charge
                    self.charges.append(chrg)

        if (
            real_resname == "ASP" or real_resname == "ASH" or real_resname == "ASX"
        ):  # regardless of protonation state, assume it's charged. This based on "The Cation-Pi Interaction," which suggests protonated state would be stabalized.
            charge_pt = point(0.0, 0.0, 0.0)
            count = 0.0
            indices = []
    ...
    class charged:
        def __init__(self, coordinates, indices, positive):
            ...
            self.positive = (
                positive  # true or false to specifiy if positive or negative charge
            )
            ...
            if len(ring1) != 0:
                for ring_index_2 in range(len(AllRings)):
                    if ring_index_1 != ring_index_2:
                        ...
                        if len(ring2) != 0:
                            if self.set1_is_subset_of_set2(ring1, ring2) is True:
    ...
    def assign_aromatic_rings_from_protein_process_residue(self, residue, last_key):
        ...
        if real_resname == "TRP":
            ...
            for index in residue:  # written this way because order is important
                atom = self.AllAtoms[index]
                if atom.atomname.strip() == "CE3":
                    indicies_of_ring.append(index)
            for index in residue:  # written this way because order is important
                atom = self.AllAtoms[index]
                if atom.atomname.strip() == "CZ3":
                    indicies_of_ring.append(index)
            for index in residue:  # written this way because order is important
                atom = self.AllAtoms[index]
                if atom.atomname.strip() == "CH2":
                    indicies_of_ring.append(index)
            for index in residue:  # written this way because order is important
                atom = self.AllAtoms[index]
                if atom.atomname.strip() == "CZ2":
                    indicies_of_ring.append(index)
            ...
            for CA_atom_index in CA_list:
                ...
                if CA_atom.structure == "BETA":  # so it's in a beta sheet
                    ...
                    for other_CA_atom_index in CA_list:
                        if (
                            other_CA_atom_index != CA_atom_index
                            ...
                            if (
                                ...
                                if (
                                    other_CA_atom.chain == CA_atom.chain
...
class MathFunctions:
    ...
    def project_point_onto_plane(
        self, apoint, plane_coefficients
    ):  # essentially finds the point on the plane that is closest to the specified point
...
class binana:
    ...
    def hashtable_entry_add_one(
        self, hashtable, key, toadd=1
    ...
    def __init__(
        ...
        parameters,
        line_header,
        actual_filename_if_ligand_is_list="",
        ...
        if (
            actual_filename_if_ligand_is_list != ""
        ):  # so a list was passed as the ligand
            ...
            for line in ligand_pdbqt_filename:
                if not "MODEL" in line and not "ENDMDL" in line:
                    f.write(line)
            f.close()
            ...
            for receptor_atom_index in receptor.AllAtoms:
                ...
                if dist < 2.5:  # less than 2.5 A
                    ...
                    self.hashtable_entry_add_one(
                        ligand_receptor_atom_type_pairs_less_than_two_half,
                        self.list_alphebetize_and_combine(list),
                    )
                ...
                if dist < 4.0:
                    ...
                    if receptor_atom.structure == "ALPHA":
                        ...
                        pdb_contacts_other_2nd_structure.AddNewAtom(
                ...
                if dist < 4.0:
                    ...
                    if (ligand_atom.element == "O" or ligand_atom.element == "N") and (
                        ...
                        for atm_index in ligand.AllAtoms:
                            if (
                                ligand.AllAtoms[atm_index].element == "H"
                            ):  # so it's a hydrogen
                                if (
                                    ligand.AllAtoms[atm_index].coordinates.dist_to(
                        ...
                        for atm_index in receptor.AllAtoms:
                            if (
                                receptor.AllAtoms[atm_index].element == "H"
                            ):  # so it's a hydrogen
                                if (
                                    receptor.AllAtoms[atm_index].coordinates.dist_to(
        ...
        for aromatic1 in ligand.aromatic_rings:
            for aromatic2 in receptor.aromatic_rings:
                ...
                if dist < 7.5:  # so there could be some pi-pi interactions.
                    ...
                    if (
                        math.fabs(angle_between_planes - 0) < 30.0
                        or math.fabs(angle_between_planes - 180) < 30.0
                    ):  # so they're more or less parallel, it's probably pi-pi stackingoutput_dir
                        ...
                        for ligand_ring_index in aromatic1.indices:
                            ...
                            pt_on_receptor_plane = (
                                self.functions.project_point_onto_plane(
                        ...
                        if pi_pi is True:
                            structure = receptor.AllAtoms[
                                aromatic2.indices[0]
                            ].structure
                            if structure == "":
                                structure = "OTHER"  # since it could be interacting with a cofactor or something
                            ...
                            for index in aromatic2.indices:
                                pdb_pistack.AddNewAtom(
                                    receptor.AllAtoms[index].copy_of()
                                )

                            self.hashtable_entry_add_one(PI_interactions, key)

                    elif (
                        math.fabs(angle_between_planes - 90) < 30.0
                        or math.fabs(angle_between_planes - 270) < 30.0
                    ):  # so they're more or less perpendicular, it's probably a pi-edge interaction
                        ...
                        if (
                            min_dist <= 5.0
                        ):  # so at their closest points, the two rings come within 5 A of each other.
        ...
        for aromatic in receptor.aromatic_rings:
            for charged in ligand.charges:
                if charged.positive is True:  # so only consider positive charges
                    if (
                        charged.coordinates.dist_to(aromatic.center) < 6.0
        ...
        for (
            aromatic
        ) in ligand.aromatic_rings:  # now it's the ligand that has the aromatic group
        ...
        for receptor_charge in receptor.charges:
            for ligand_charge in ligand.charges:
                if (
                    ligand_charge.positive != receptor_charge.positive
                ):  # so they have oppositve charges
                    if (
                        ligand_charge.coordinates.dist_to(receptor_charge.coordinates)
                        < 5.5
                    ):  # 4  is good cutoff for salt bridges according to "Close-Range Electrostatic Interactions in Proteins", but looking at complexes, I decided to go with 5.5 A
                        structure = receptor.AllAtoms[
        ...
        # Now get vina
        vina_output = getCommandOutput2(
            parameters.params["vina_executable"]
            + " --score_only --receptor "
            + receptor_pdbqt_filename
            + " --ligand "
            + ligand_pdbqt_filename
        )
        ...
        for item in vina_output:
            ...
            if "Affinity" in item:
                vina_affinity = float(
                    item.replace("Affinity: ", "").replace(" (kcal/mol)", "")
                )
            if "gauss 1" in item:
        ...
        for key in PI_interactions:
            ...
            together = (
                key + "_" + str(value)
            )  # not that this is put together strangely!!!
        ...
        for key in data["ligand_receptor_atom_type_pairs_less_than_two_half"]:
            if not key in self.ligand_receptor_atom_type_pairs_less_than_two_half:
                print(
                    "\tWARNING: Atoms of types "
                    + key.replace("_", " and ")
                    + " come within 2.5 angstroms of each other."
                )
                print(
                    "\t  The neural networks were not trained to deal with this juxtaposition,"
                )
                print("\t  so it will be ignored.")
                self.error = True
            else:
                self.ligand_receptor_atom_type_pairs_less_than_two_half[key] = data[
                    "ligand_receptor_atom_type_pairs_less_than_two_half"
                ][key]
        ...
        for key in data["ligand_receptor_atom_type_pairs_less_than_four"]:
            if not key in self.ligand_receptor_atom_type_pairs_less_than_four:
                print(
                    "\tWARNING: Atoms of types "
                    + key.replace("_", " and ")
                    + " come within 4 angstroms of each other."
                )
                print(
                    "\t  The neural networks were not trained to deal with this juxtaposition,"
                )
                ...
                self.ligand_receptor_atom_type_pairs_less_than_four[key] = data[
                    "ligand_receptor_atom_type_pairs_less_than_four"
                ][key]
        ...
        for key in data["ligand_atom_types"]:
            if not key in self.ligand_atom_types:
                print(
                    "\tWARNING: The ligand contains an atoms of type "
                    + key
                    + ". The neural networks"
                )
                print(
                    "\t  were not trained to deal with this ligand atom type, so it will be ignored."
                )
        ...
        for key in data["ligand_receptor_atom_type_pairs_electrostatic"]:
            if not key in self.ligand_receptor_atom_type_pairs_electrostatic:
                print(
                    "\tWARNING: Atoms of types "
                    + key.replace("_", " and ")
                    + ", which come within 4 angstroms of each"
                )
                print(
                    "\t  other, may interact electrostatically. However, the neural networks"
                )
                print(
                    "\t  were not trained to deal with electrostatic interactions between atoms"
                )
                print("\t  of these types, so they will be ignored.")

                self.error = True
            else:
                self.ligand_receptor_atom_type_pairs_electrostatic[key] = data[
                    "ligand_receptor_atom_type_pairs_electrostatic"
                ][key]
        ...
        alpha_tmp = (
            self.active_site_flexibility["BACKBONE_ALPHA"]
        ...
        self.active_site_flexibility_by_backbone_or_sidechain = {
            "BACKBONE": backbone_tmp,
            "SIDECHAIN": sidechain_tmp,
        }
        ...
        hdonor_ligand = (
            self.hbonds["HDONOR-LIGAND_BACKBONE_ALPHA"]
        ...
        self.hbonds_by_location_of_hdonor = {
            "LIGAND": hdonor_ligand,
            "RECEPTOR": hdonor_receptor,
        }
        ...
        self.hbonds_by_backbone_or_sidechain = {
            "BACKBONE": hbond_backbone,
            "SIDECHAIN": hbond_sidechain,
        }
        ...
        alpha_tmp = (
            self.hydrophobics["BACKBONE_ALPHA"] + self.hydrophobics["SIDECHAIN_ALPHA"]
        )
        beta_tmp = (
            self.hydrophobics["BACKBONE_BETA"] + self.hydrophobics["SIDECHAIN_BETA"]
        )
        other_tmp = (
            self.hydrophobics["BACKBONE_OTHER"] + self.hydrophobics["SIDECHAIN_OTHER"]
        )
        ...
        backbone_tmp = (
            self.hydrophobics["BACKBONE_ALPHA"]
        ...
        self.pi_cation = {
            "LIGAND-CHARGED_ALPHA": 0,
            "LIGAND-CHARGED_BETA": 0,
            "LIGAND-CHARGED_OTHER": 0,
            "RECEPTOR-CHARGED_ALPHA": 0,
            "RECEPTOR-CHARGED_BETA": 0,
            "RECEPTOR-CHARGED_OTHER": 0,
        }
        for key in pi_cation_tmp:
            self.pi_cation[key] = pi_cation_tmp[key]

        pi_cation_ligand_charged = (
            self.pi_cation["LIGAND-CHARGED_ALPHA"]
            + self.pi_cation["LIGAND-CHARGED_BETA"]
            + self.pi_cation["LIGAND-CHARGED_OTHER"]
        )
        pi_cation_receptor_charged = (
        ...
        self.pi_cation_by_structure = {
            "ALPHA": pi_cation_alpha,
            "BETA": pi_cation_beta,
            "OTHER": pi_cation_other,
        }
        ...
        all = (
            self.salt_bridges["ALPHA"]
            + self.salt_bridges["BETA"]
            + self.salt_bridges["OTHER"]
        )
        ...
        for index in range(len(parameters)):
            ...
            if item[:1] == "-":  # so it's a parameter key value
                ...
                if key == "help":
                    ...
                    print(
                        textwrap.fill(
                            "-ligand: File name of the ligand PDBQT file. AutoDock Vina output files, typically containing multiple poses, are also permitted."
                        )
                        + "\n"
                    )
                    print(
                        textwrap.fill(
                            "-vina_executable: The location of the AutoDock Vina 1.1.2 executable. If you don't wish to specify the location of this file every time you use NNScore 2.02, simply edit the vina_executable variable defined near the begining of the NNScore2.py script."
                        )
                        + "\n"
                    )
                    ...
                    print(
                        textwrap.fill(
                            "NNScore 2.02 evaluates each of the ligand poses contained in the file specified by the -ligand tag using 20 distinct neural-network scoring functions. The program then seeks to identify which of the poses has the highest predicted affinity using several metrics:"
                        )
                        + "\n"
                    )
                    print(
...
def run():
    ...
    best_scores = sorted(
        scores, key=lambda score: score[2], reverse=True
    )  # sort by score
